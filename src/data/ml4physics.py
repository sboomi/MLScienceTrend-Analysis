...
def download_ml4physics_pdf(year: int, idx: int, dest_folder: Path, chunk_size: int = 512) -> None:
    dest_file = dest_folder / f"ml4physics_{year}_{idx}.pdf"
    if not dest_file.exists():
        if year == 2017:
            url = f"{ML4PHYSICS_URL}{year}/files/nips_dlps_{year}_{idx}.pdf"
        else:
            url = f"{ML4PHYSICS_URL}{year}/files/NeurIPS_ML4PS_{year}_{idx}.pdf"
        ...
        if r.status_code == 200:
            with open(dest_file, "wb") as f:
                for chunk in r.iter_content(chunk_size):
                    f.write(chunk)
            logger.info(f"{dest_file.as_posix()} successfully written.")
            return
        logger.error(f"There was a problem with the network.\nStatus code: {r.status_code}")
        return
    logger.warning(f"{dest_file} already exists")


def download_all_ml4physics_pdf(dest_folder: Path, chunk_size: int = 512) -> None:
    df = extract_ml4physics()
    for index, row in df.iterrows():
        logger.info(f"Downloading {row.title}")
        download_ml4physics_pdf(row.year, index, dest_folder, chunk_size)
    logger.info("Done!")
